...
class RushSelect extends ArrayPrompt implements IRushSelect {
  constructor(options = {}) {
    ...
    // for padding
    this.shortestPackageNameLength = this.choices.reduce((shortest: number, curr: ChoiceInPrompt) => {
  ...
  getSortedChoices(choices: Array<ChoiceInPrompt>): Array<ChoiceInPrompt> {
    return choices.sort((a: ChoiceInPrompt, b: ChoiceInPrompt) => {
      ...
      return customSortOrder(a.customSortText || a.category) < customSortOrder(b.customSortText || b.category)
        ? -1
        : 1;
    });
  }
  ...
  onKeyPress(ch: string, key: KeyPressEvent): void {
    ...
    if (typeof ch === 'string' && key.raw === key.sequence && !key.ctrl && !key.meta && !key.option) {
  ...
  isValidScaleItem(scaleItemName: ScaleWithName, choice: ChoiceInPrompt): boolean {
    return scaleItemName === this.options.ignoreText || this.isScriptAvailable(scaleItemName, choice);
  }
  ...
  getNextIndexThatHasAvailableScript(direction: 'left' | 'right', choice: ChoiceInPrompt): number {
    ...
    while (isIndexWithinBounds(nextIndex) && !this.isValidScaleItem(this.scale[nextIndex], choice)) {
  ...
  scaleIndicator(choice: ChoiceInPrompt, item: ScaleWithIndex, choiceIndex: number): string {
    ...
    const isIgnoreScript = this.getChoiceAvailableScriptIndexes(choice)[0].index === choice.scaleIndex;
  ...
  async renderChoice(choice: ChoiceInPrompt, i: number, bulletIndentation = false): Promise<Array<string>> {
    ...
    const pad = (str: string) => this.margin[3] + str.replace(/\s+$/, '').padEnd(this.widths[0], ' ');
    ...
    if (focused) {
      lines = lines.map((line: string) => (this.styles.hasAnsi(line) ? line : this.styles.danger(line)));
    ...
    do {
      ...
      columnSpaceRemaining =
        termColumns - stripAnsi(Array.isArray(renderedChoice) ? renderedChoice[0] : renderedChoice).length;
