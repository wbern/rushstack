...
const createRushPrompt = async (
  ...
  const rushSelect = new RushSelect({
    name: 'rush-select',
    message: 'Select what to run. Use left/right arrows to change options, Enter key starts execution.',
    messageWidth: 150,
    margin: [0, 1, 0, 0],
    ...
    if (!nonZeroExit) {
      ...
      if (scripts.rushBuild) {
        if (scripts.rushBuild.script === 'smart' && packagesThatWillRunScripts.length > 0) {
          ...
          rushBuildProcess = spawnStreaming(executable, args, { cwd: getRushRootDir() }, 'rush rebuild');
        }
